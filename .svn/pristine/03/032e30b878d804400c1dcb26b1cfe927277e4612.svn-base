		...
		<div id="lookup_tb" style="padding: 5px; height: auto">
			<div class="condition-container" style="width:80%;height:100px;">
				<form id="lookup_form" action="" method="post">
					<input type="hidden"  name="allot_equipIds" id="allot_equipIds" value="${equipIds}" />
					<input type="hidden"  name="allot_type" id="allot_type" value="${type}" />
	...
	function selectStaff()
		{
			...
			if (count == 0) {
				...
				var equipIds = $("input[name='allot_equipIds']").val();
				var type = $("input[name='allot_type']").val();
				var staff_id = selected[0].STAFF_ID;
				$.ajax({
					type : 'POST',
					url : webPath + "equip/allotEquip.do",
					data : {
						equipIds:equipIds,
						type : type,
						staff_id : staff_id
