...
<html>
	...
	<body style="padding: 3px; border: 0px">
		...
		<div id="tb" style="padding: 5px; height: auto">
				...
				<form id="form" action="" method="post">
					<table class="condition">
						...
						<tr>						
							...
							<td width="25%" align="left">
								<div class="condition-text-container">
									<input name="checkport" id="checkport" class="condition-text" />
								</div>
							</td>
							...
							<td width="25%" align="left">
								<div class="condition-text-container">
									<input name="checkrate" id="checkrate" class="condition-text" />
								</div>
							</td>
							<td width="10%">
								排序条件：
							...
							<td width="20%">
								<select name="orderby" id="orderby" class="condition-select">
									<option value="0">
										按完成时间
									</option>
									<option value="1">
			...
			<div class="btn-operation-container">
			    <div style="float: right;" class="btn-operation" onClick="exportExcelByEqp()">导出</div>
				...
				<div style="float:right;" class="btn-operation" onClick="doTask()">
					二次复查派发
				</div>
	...
	<script type="text/javascript">
		...
		function searchData() {
			...
			var checkport = $("input[name='checkport']").val();
			var checkrate = $("input[name='checkrate']").val();
			...
			$('#dg').datagrid({
				autoSize : true,
				toolbar : '#tb',
				url : webPath + "DoneTask/querySecondTask.do",
				queryParams : obj,
				method : 'post',
				pagination : true,
				pageNumber : 1,
	...
	function getSonAreaList() {
		...
		/* 	$.messager.confirm('系统提示', '您确定要导出信息吗?', function (r) {	
      		...
      		window.open(webPath+ "DoneTask/exportExcel.do?taskIds=" + taskIds+"&sblx="+sblx+"&whwg="+whwg+"&equipmentCode="+equipmentCode+"&STATUS_ID="+STATUS_ID+"&TASK_NAME="+TASK_NAME+"&SON_AREA_ID="+SON_AREA_ID+"&TASK_TYPE="+TASK_TYPE+"&START_TIME="+START_TIME+"&COMPLETE_TIME="+COMPLETE_TIME+"&AREA_ID="+AREA_ID+"&C_START_TIME="+
      			C_START_TIME+"&C_COMPLETE_TIME="+C_COMPLETE_TIME+"&INSPECTOR="+INSPECTOR+"&pos_eqpCode="+pos_eqpCode);
		...
		/* 	$.messager.confirm('系统提示', '您确定要导出信息吗?', function (r) {
        	...
        	window.open(webPath+"DoneTask/exportSecondTaskExcelByEqp.do?taskIds="+taskIds+"&sblx="+sblx+"&whwg="+whwg+"&area_id="+area_id+"&equipmentCode="+equipmentCode+"&STATUS_ID="+STATUS_ID+"&TASK_NAME="+TASK_NAME
        	+"&SON_AREA_ID="+SON_AREA_ID+"&TASK_TYPE="+TASK_TYPE+"&START_TIME="+START_TIME
        	+"&COMPLETE_TIME="+COMPLETE_TIME+"&C_START_TIME="+C_START_TIME+"&C_COMPLETE_TIME="+C_COMPLETE_TIME+"&INSPECTOR="+INSPECTOR+"&orderby="+orderby+"&checkrate="+checkrate+"&checkport="+checkport);
		}
