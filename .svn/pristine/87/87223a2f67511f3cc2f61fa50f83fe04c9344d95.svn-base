...
<div class="easyui-tabs" id="pointTabs1" style="width:900px;height:540px;float: left;margin-left: 10px;overflow:auto;  ">
	...
	<script type="text/javascript">
		...
		$.ajax({
			...
			success : function(json) 
			{
				...
				$("select[name='son_area'] option[value="+'${SON_AREA_ID}'+"]").attr("selected",true);
			}
		});
	}
		...
		function searchPointData() {
			...
			$('#dg').datagrid({
				...
				autoSize : false,
				toolbar : '#tb',
				url : webPath + "/CablePlan/searchPointData.do",
				queryParams : obj,
				method : 'post',
				pagination : true,
				pageNumber : 1,
...
function searchData() {
		...
		function save(){
			...
			var dept_name = $.trim($("select[name='dept_name']").val());
			...
			$.ajax({
				type : 'POST',
				url : webPath + "/CablePlan/saveEditPointPlan.do",
				data : {
					areaId : areaId,
					plan_id:plan_id,
					plan_name:plan_name,
	...
	/* 	    $("#dept_name").combobox({
			...
			success : function(json) 
			{
				...
				for ( var i = 0; i < result.length; i++) {
					if (result[i].DEPT_NO ==select_dept_no)
					{
						$("select[name='dept_name']").append("<option value=" + result[i].DEPT_NO + " selected='selected'>"
										+ result[i].DEPT_NAME + "</option>");
						$("select[name='plan_dept_name']").append("<option value=" + result[i].DEPT_NO + " selected='selected'>"
										+ result[i].DEPT_NAME + "</option>");
					}
					else{
