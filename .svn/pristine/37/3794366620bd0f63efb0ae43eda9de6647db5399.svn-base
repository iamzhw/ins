			...
			<table id="tbl1">
					...
					<td>
						<div class="condition-text-container">
							<input class="condition-text easyui-textbox condition"
								type="text" id="steppart_name" name="steppart_name" value=""
				...
				<tr>
					...
					<td>
						<div class="condition-text-container">
							<input class="condition-text easyui-textbox condition"
								type="text" id="start_equip" name="start_equip" value=""
								readonly="true" />
						</div></td>
				</tr>
				<tr>
					...
					<td>
						<div class="condition-text-container">
							<input class="condition-text easyui-textbox condition"
								type="text" id="end_equip" name="end_equip" value=""
								readonly="true" />
						</div>
					</td>
				</tr>
...
<!-- 								<c:if test="${empty trunkcircle}"> -->
<!-- 							      <option value="">请选择</option> -->
<!-- 								</c:if> -->
<!-- 								<c:if test="${not empty trunkcircle}">   -->
								  <option value="${trunkcircle}">${trunkcircle}个月</option>
<!-- 								</c:if> -->
...
<!-- 								<option value="12">一年</option> -->
				...
				<tr>
					<td>巡线人：</td>
					<td><select name="inspect_id" id="inspect_id"
...
function addHandler(content,marker){ 
	marker.addEventListener("click",function(e){//鼠标悬浮上设施点，设施点事件绑定
			if(marker.is_equip == 1 && marker.is_part==0){
			openInfo(content,e);
			}
		}
	);
...
// 			async: false,
	...
	$(is_equip_list).each(function(){
		...
		//如果是路由（is_equip=1）并且未分配步巡段(is_part=0)的才给展示右键菜单
		if(this.IS_EQUIP == 1 && this.IS_PART==0){
		...
		var content = "设施id:"+this.EQUIP_ID+"<br/>编号:"+this.EQUIP_CODE+"<br/>描述："+this.DESCRIPTION+"<br/>排序："+this.ORDER;
		      ...
		      success : function(data) {
		        if(data.status){
					...
					if (r) {
						$.ajax({
							type : 'POST',
							url : webPath + "/StepPart/saveStepPart.do",
							data : {
