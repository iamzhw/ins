...
<body style="padding: 3px; border: 0px">
	...
	<div class="easyui-tabs" id="pointTabs"
		...
		<div title="填写计划信息" style="padding: 10px" data-options="selected:true">
			<form id="form" action="" method="post">
				...
				<input type="hidden" id="son_area_id_edit" value="${SON_AREA_ID}" />
				<input type="hidden" id="whwg_edit" value="${WHWG}" />
				<input type="hidden" id="check_staff_edit" value="${CHECK_STAFF}" />
				...
				<table class="condition" style="width: 100%;">
					<tr>
						...
						<td width="20%">
							<select id="show_area" name="show_area" class="condition-select" onchange="getSonAreaList()">
									<option value="">
										请选择
									</option>
					...
					<tr>
						...
						<td>
							...
							value="1" data-options="increment:1" style="width: 155px;" value="${PLAN_FREQUENCY}"></input>
						</td>
					</tr>
						...
						<td style="width: 70%;">
							<div class="condition-text-container">
								<input id="plan_name" name="plan_name" type="text"
									class="condition-text" readonly="readonly" value="${PLAN_NAME}"/>
							</div>
						</td>
					</tr>
					<tr>
	...
	<div class="easyui-tabs"
		style="width: 650px; height: 540px; float: left;">
		<div title="设备展示" style="padding: 10px" data-options="selected:true">
			...
			<table id="dg" title="【设备管理】" style="width:98%;height:480px">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("select[name='plan_type'] option[value="+'${PLAN_TYPE}'+"]").attr("selected",true);
			$("input[name='plan_start_time']").val('${PLAN_START_TIME}');
			$("input[name='plan_end_time']").val('${PLAN_END_TIME}');
			$("select[name='show_area'] option[value="+'${AREA_ID}'+"]").attr("selected",true);
			if('1'=='${PLAN_TYPE}'){
				$("#check_staff_tr").hide();
			}
			getSonAreaList1();
		...
		function save() {
			...
			$.ajax({
				type : 'POST',
				url : webPath + "/patrolPlan/saveEditPatrolPlan.do",
				data : {
					areaId : areaId,
					sonAreaId : sonAreaId,
					whwg : whwg,
		...
		function getSonAreaList1() {
			...
			$.ajax({
				...
				success : function(json) 
				{		
					var son_area_id_edit = $("#son_area_id_edit").val();
					var result = json.sonAreaList;
					$("select[name='SON_AREA_ID'] option").remove();
					$("select[name='SON_AREA_ID']").append("<option value=''>请选择</option>");
					$("select[name='WHWG'] option").remove();
					$("select[name='WHWG']").append("<option value=''>请选择</option>");
					for ( var i = 0; i < result.length; i++) {
						if(result[i].AREA_ID == son_area_id_edit){
							$("select[name='SON_AREA_ID']").append("<option value=" + result[i].AREA_ID + " selected='selected'>"+ result[i].NAME + "</option>");
						}else{
							$("select[name='SON_AREA_ID']").append("<option value=" + result[i].AREA_ID + ">"+ result[i].NAME + "</option>");					
						}
					}
		...
		function getGridList1() {
			var areaId = $("#show_area").val();
			var sonAreaId = $("#son_area_id_edit").val();
			$.ajax({
				type : 'POST',
				url : webPath + "General/getGridList.do",
				data : {
				...
				success : function(json) 
				{
					var whwg = $("#whwg_edit").val();
					var result = json.gridList;
					$("select[name='WHWG'] option").remove();
					$("select[name='WHWG']").append("<option value=''>请选择</option>");
					for ( var i = 0; i < result.length; i++) {
						if(result[i].GRID_ID==whwg){
							$("select[name='WHWG']").append("<option value=" + result[i].GRID_ID + " selected='selected'>"+ result[i].GRID_NAME + "</option>");
						}else{
							$("select[name='WHWG']").append("<option value=" + result[i].GRID_ID + ">"+ result[i].GRID_NAME + "</option>");
						}
					}
		...
		function getStaffList1(){
			var whwgId = $("#whwg_edit").val();
			$.ajax({
				type : 'POST',
				url : webPath + "patrolPlan/getStaffListByWHWGId.do",
				data : {
				...
				success : function(json) 
				{
					var check_staff = $("#check_staff_edit").val();
					var result = json.staffList;
					$("select[name='STAFF_NAME'] option").remove();
					$("select[name='STAFF_NAME']").append("<option value=''>请选择</option>");
					for ( var i = 0; i < result.length; i++) {
						if(result[i].STAFF_ID==check_staff){
							$("select[name='STAFF_NAME']").append("<option value=" + result[i].STAFF_ID + " selected='selected'>"+ result[i].STAFF_NAME + "</option>");
						}else{
							$("select[name='STAFF_NAME']").append("<option value=" + result[i].STAFF_ID + ">"+ result[i].STAFF_NAME + "</option>");
						}
					}
