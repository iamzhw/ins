...
<body style="padding: 3px; border: 0px">
				...
				<tr>
					...
					<td><div class="condition-text-container">
							<input class="condition-text easyui-validatebox condition"
								type="text" value="${cable_name}" readonly="readonly" />
						</div></td>
				</tr>
				<tr>
					<td>中继段名称：</td>
					<td><div class="condition-text-container">
							<input class="condition-text easyui-validatebox condition"
								type="text" value="${relay_name}" readonly="readonly" />
						</div></td>
				</tr>
			</table>
		</form>
	...
	<div style="height: 40px;">
		<div style="margin-left: -5px; margin-top:10px;" class="btn-operation"
		      onClick="lastPage();">上一页/第${page}页</div>
		<select name="pageSize"  style="margin-left: -4px; margin-top:11px; width: 7%;" 
		         class="condition-select" onchange="changePageSize(this.value)">
				<option value="${pageSize}">当前${pageSize}条</option>
		...
		<div style="margin-left: 94px;margin-top:-27px;" class="btn-operation"
			onClick="nextPage();">下一页/共${count}页</div>
	</div>
...
//设置一个延迟的，已解决单击双击冲突问题
 clickTimeout = {  
            ...
            set: function (fn) {  
                var that = this;  
                that.clear();  
                that.timeout = window.setTimeout(fn, 300);  
            },  
            clear: function () {  
                var that = this;  
                if (that.timeout) {  
...
function addHandler(content,marker){ 
	/* marker.addEventListener("mouseover",function(e){//鼠标悬浮上设施点，设施点事件绑定
			//openInfo(content,e);
		}
	); */
	marker.addEventListener("dblclick",function(e){//鼠标增加双击事件,点击出现属性窗口
		 clickTimeout.clear();  
		 equipid=e.currentTarget.equip_id;
		$("#edit_win").window({
            title: "【步巡点信息】",
            href : webPath + "StepEquip/edit.do?equip_id="+equipid,
			width : 650,
			height : 400,
            zIndex: 2,
            region: "center",
...
// 		 console.log(e.currentTarget.equip_id);
		...
		if(!status){  //不在points中，进行线段关联操作与事件绑定操作
			...
			$(modelResultEquipList).each(function(){
				if(this.LONGITUDE==mk.getPosition().lng && this.LATITUDE==mk.getPosition().lat){
					$("#dg").append("<tr><td>"+this.EQUIP_ID+"</td><td>"+this.EQUIP_CODE+"</td><td>"+this.EQUIP_TYPE+"</td><td>"+this.LONGITUDE+","+this.LATITUDE+"</td></tr>");
					return;
				}				
			});
			$("#dg tr:not(:first)").bind('click',showAnimation);
		}else{
//		    20170113 孙敏  点击第一个或者最后一个点修改过于麻烦，一步小心就会跳到下一页。所以修改成按钮
...
// 					return ;
// 				}
// 				location.href="changeEquipInit.do?page="+page+"&cable_id="+cable_id+"&relay_id="+relay_id+"&area_id="+area_id;
...
// 		$(point).attr("equip_id",this.EQUIP_ID);
// 		$(point).attr("equip_type",this.EQUIP_TYPE);
// 		console.log(point);
...
// 		console.log(marker);
		var content = "设施id:"+this.EQUIP_ID+"<br/>设施编号："+this.EQUIP_CODE+"<br/>设施类型："+this.EQUIP_TYPE+"<br/>设施描述:"+this.DESCRIPTION+"<br/>排序："+this.ORDER;
...
// 		$(point).attr("equip_type",this.EQUIP_TYPE);
    ...
    function resultListToCommit(){
    	...
    	$.ajax({
			url : webPath + "/StepEquip/saveEquipByChange.do",// 跳转到 action    
			data : {ids:resultEquipList.toString(),page:page,relay_id:relay_id,area_id:area_id,cable_id:cable_id,pageSize:pageSize},
			type : 'post',
			cache : false,
			dataType : 'json',
			async: false,
			success : function(data) {
				if(data.status){
					...
					location.href="changeEquipInit.do?page="+1+"&cable_id="+cable_id+"&relay_id="+relay_id+"&area_id="+area_id+"&pageSize="+pageSize;
				}else{
    ...
    function updateForm() {
			...
			if (pass) {
				$.messager.confirm('系统提示', '您确定要更新信息點信息吗?', function(r) {
					if (r) {
						var equip_id = $("input[name='equip_id']").val();
						var equip_code = $("input[name='equip_code']").val();
						var depth = $("input[name='depth']").val();
						var longitude = $("input[name='longitude']").val();
						var latitude = $("input[name='latitude']").val();
						var equip_address = $("input[name='equip_address']").val();
						var description = $("input[name='description']").val();
						var owner_name = $("input[name='owner_name']").val();
						var owner_tel = $("input[name='owner_tel']").val();
						var protecter = $("input[name='protecter']").val();
						var protect_tel = $("input[name='protect_tel']").val();
						
						$.ajax({
							type : 'POST',
							url : webPath + "StepEquip/update.do",
							data : {
								equip_id:equip_id,
    ...
    function lastPage(){
       ...
       location.href="changeEquipInit.do?page="+page+"&cable_id="+cable_id+"&relay_id="+relay_id+"&area_id="+area_id+"&pageSize="+pageSize;
    }
    ...
    function changePageSize(changePageSize){
       //每次修改完成直接重置回第一页
       page = 1;
