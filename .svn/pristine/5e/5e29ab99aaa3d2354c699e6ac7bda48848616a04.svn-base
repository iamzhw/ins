...
public class MatchSiteGtThread implements Runnable {
	...
	public MatchSiteGtThread(LineSiteInterfaceDao lineSiteInterfaceDao, ParamService paramService, String jsonStr) {
		this.lineSiteInterfaceDao = lineSiteInterfaceDao;
	...
	public void run() {
		try {
			saveAutoGtTrack(jsonStr);
		} catch (Exception e) {
			paramService.saveLogInfo("MatchSiteGtThread", "saveAutoGtTrack(String jsonStr)", "ERROR", jsonStr);
		}
	}

	public void saveAutoGtTrack(String jsonStr) {
		...
		// 初始化结果json对象
		saveGtTrack(userId, areaId, uploadTime, trackArray);
	}

	private void saveGtTrack(String userId, String areaId, String uploadTime, JSONArray trackArray) {
...
//			}
			List<Map<String, Object>> maxTracks = lineSiteInterfaceDao.getMaxGtTrackByUser(params);
			if (null != maxTracks && maxTracks.size() > 0) {// 获取最近信号轨迹
				track1 = maxTracks.get(0);
			}

			for (int i = 0; i < trackArray.size(); i++) {
				track2 = analysisTrack(trackArray.getJSONObject(i), userId, areaId, uploadTime, "0");
				filterTrackMaxDistance(track1, track2);
				if (lineSiteInterfaceDao.getUserGtTrackTime(track2) == 0) {
					lineSiteInterfaceDao.saveAutoGtTrack(track2);// 保存自动上传轨迹
				}
				track1 = track2;
			}
