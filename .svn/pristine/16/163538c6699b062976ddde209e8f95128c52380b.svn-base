...
import com.util.webservice.AsigAxisServiceServiceLocator;
import com.util.webservice.AsigAxisServiceSoapBindingStub;
...
public class SendMessageUtil {
	...
	public static void sendMessageByCallProvinceService(String phoneNum, String sendMsg) throws Exception {
		try {
			...
			// String checkNbr =
			// (String)basedao.get("StaffManage.getMessageCheckNbr", param);
			...
			if (iResult != 0) {
				...
				// baseMethodService.saveLog("调用省公司短信接口发送短信", "短信发送失败，接口返回内容：" +
				// smsg, "1", null);
				throw new Exception("调用省公司接口发送短信失败"); // 发送失败则抛出异常，外层捕获异常后使用其他方式发送短信
			}
		} catch (Exception e) {
			e.printStackTrace();
			// baseMethodService.saveLog("调用省公司短信接口发送短信", "发生异常,异常内容：" +
			// e.getMessage(), "1", null);
	...
	private static boolean isChinaTelcomPhoneNum(String phoneNum) {
		...
		if (phoneNum != null && phoneNum.length() > 0) {
			if (phoneNum.startsWith("133") || phoneNum.startsWith("189") || phoneNum.startsWith("153")
					|| phoneNum.startsWith("180") || phoneNum.startsWith("177")) {
				isChinaTelcom = true;
			}
		}
	 ...
	 * sendMessage("15366165868"
	 * ,"测试员，您好。您于"+DateUtil.getCurrentTime()+"收到一条测试任务，请及时执行。【分公司缆线巡检】"); }
	...
	public static boolean sendMessage(String phoneNum, String sendMsg) {
		try {
			if (PropertiesUtil.getPropertyBoolean("sendSmsFlag")) {
				...
				reqInfo.append("<content>").append(phoneNum).append("</content>");
				reqInfo.append("</message>");
				reqInfo.append("</messages>");
				reqInfo.append("</reqInfo>");
	...
	public static boolean sendMessageList(List phoneNumList, List messageContentList) {
		try {
			if (PropertiesUtil.getPropertyBoolean("sendSmsFlag")) {
				...
				reqInfo.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
				reqInfo.append("<reqInfo>");
				if (null != phoneNumList && phoneNumList.size() > 0 && null != messageContentList
						&& messageContentList.size() > 0 && messageContentList.size() == phoneNumList.size()) {
					reqInfo.append("<messages>");
	 ...
	 * @return
	 */
	public static boolean sendMessageToMore(List<String> phoneNumList, String messageContent) {
		try {
			// 拼裝xml
			StringBuffer reqInfo = new StringBuffer();
			reqInfo.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
