...
<script type="text/javascript">
  	...
  	function searchData(type){
 				...
 				columns : [ [ 
          		{
 						...
 						formatter : function(value, rec,
								index) {
							return "<a  style='color:blue' onclick='getPhotoUI(\""
							+ rec.OUT_RECORD_ID
							+ "\",3)'>图片</a>";
		...
		var cid=${AFFECTED_FIBER};
		function getRelay() {
			
			$('#win_affactedCable').window({
				title : "【选择中继段】",
				href : webPath + "mainOutSiteController/getRelayByCableId.do?cid="+cid,
				width : 250,
				height : 300,
				zIndex : 2,
				region : "center",
		...
		function showTheCheckedRelay() {
			var relay_id = '';
			var relay_name = '';

			$("input[name='tbrelay']").each(function() {
					if (this.checked) {
						var v = this.value.split(":");
						relay_id = relay_id + v[0] + ',';
						relay_name = relay_name + v[1] + ',';
					}

				});

			$("#relay_name").val("");
			$("#relay_name").val(relay_name);
			if(relay_id!=''){
				relay_part=relay_id.substring(0,relay_id.length-1);
			}
			$("#relay_part").val(relay_part);
		};
		...
		function czsEditUI(){
		 ...
		 function getPhotoUI(site_id,type){
				$('#win_jctp').window({
						title : "【检查图片】",
						href : webPath+ "mainOutSiteController/getPhoto.do?site_id="+site_id+"&photo_type="+type,
						width : 1000,
						height : 500,
						zIndex : 2,
						region : "center",
	...
	<div title="基本信息" data-options="closable:false,selected: true"
		...
		<form id="formEdit" method="post">
			<input type="hidden" id="out_site_id" name="out_site_id"
				...
				<input type="hidden" id="relay_part" name="relay_part" value="">
			...
			<table class="table_border">
				...
				<tr>
					...
					<td><div class="">
							经度： <input type="text" class='condition-text-container'
								value="${X}" name="x" name="x" style="width:80px" readonly="readonly">
						</div></td>
					<td><div class="">
							纬度： <input type="text" class='condition-text-container'
								value="${Y}" name="y" name="x" style="width:80px" readonly="readonly">
						</div></td>
				...
				<tr>
					...
					<td>
						<div class="">
								...
								<select
								class="condition-select" onchange="setEponsible2(this.value)"  readonly="readonly"> 
					...
					<td rowspan="2" colspan="1">
						<div class="">
							...
							<textarea class="" rows="2" cols="10" name="affactedCable"
								id="affactedCable" readonly="readonly">${AFFECTED_FIBER_NAMES}</textarea>
							<input type="button" onclick="getCable()" value="选择" />
						</div>
					</td>
				...
				<tr>
					<td>干线等级：</td>
					<td><div class="">
							<select name="fiber_level" id="fiber_level"
								class="condition-select">
								<option value="1"
				...
				<tr>
					...
					<td rowspan="1" colspan="1">
						<div class="">
							<textarea id="relay_name" name="relay_name" rows="1" cols="10" readonly="readonly" >${RELAY_NAME}</textarea>
							<input type="button" onclick="getRelay()" value="选择" />
						</div>
					</td>
				...
				<tr>
					...
					--%><td colspan="2" rowspan="1">传输局确认： <input class="" type="text"
						id="csjMakesure" name="csjMakesure" style="width:200px;border:0" /><input
						type="button" onclick="csjMakesure2()" value="确认" />
					</td>
				</tr>
				<tr>
					<td>线务段：</td>
					...
					<td>
						<div class="">
							<input type="button" onclick="" value="上传" />
						</div></td>
				</tr>
				<tr>
					<td>标石号（人孔号）：</td>
	...
	<div title="机械操作手信息" data-options="closable:false"
		...
		<div id="tb1" style="padding:5px;height:auto">
			...
			<div class="btn-operation-container">
				...
				<div style="float:left;" class="btn-operation" onClick="czsEditUI()">修改</div>

				<div style="float:left;" class="btn-operation" onClick="czsDelete()">删除</div>
