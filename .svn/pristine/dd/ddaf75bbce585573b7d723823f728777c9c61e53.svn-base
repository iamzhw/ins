...
<body>
<input name="selecttaskIds" id="selecttaskIds" value="${taskIds}" type="hidden"/>
...
function searchData1() {
	...
	$('#dg_soft').datagrid({
		...
		height : 460,
		toolbar : '#tb_soft',
		url : webPath + "DoneTask/queryHandler.do",
		queryParams : obj,
		method : 'post',
		pagination : true,
		pageNumber : 1,
...
function saveTask(){
	...
	if (count == 0) {
		...
		var taskIds = $("input[name='selecttaskIds']").val();
		$.ajax({
			type : 'GET',
			url : webPath + "DoneTask/saveTask.do?staffId="+staffId+"&taskIds="+taskIds,
			dataType : 'json',
			success : function(json) {
