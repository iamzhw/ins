/**
 * jQuery EasyUI 1.3.4 1. support datagrid autosize modified by wangyan @ 20140514
 * note: to support auto size, call $().datagrid like: $("#id").datagrid({
 * autoSize:true }); 2. support add custom word before and after the
 * rows-count-per-page selector
 * 
...
(function($) {
	function _80(_81) {
		...
		if (_83.showPageList) {
			...
			$("<td></td>")
					.append("<span>" + _83.beforePageListText + "</span>")
					.appendTo(tr);
			$("<td></td>").append(ps).appendTo(tr);
			$("<td></td>").append("<span>" + _83.afterPageListText + "</span>")
					.appendTo(tr);
...
(function($) {
	function show(el, type, _25e, _25f) {
		...
		case "fade":
			...
			break;
		case "show":
			// wangyan modified @ 20140603 for center the msgbox window
			// _25f = 999999999;
			var o = $(win);
			var w = parseInt(o.css("width").replace(/px/g, ""));
			var h = parseInt(o.css("height").replace(/px/g, ""));
			o.css("width", w);
			o.css("height", h);
			o.css("left", "50%");
			o.css("top", "50%");
			o.css("margin-left", "-" + (w / 2) + "px");
			o.css("margin-top", "-" + (h / 2) + "px");
			win.show(_25e);
			break;
		}
		var _260 = null;
	...
	function _264(_265, _266, _267) {
		...
		win.append(_266);
		if (_267) {
			// wangyan modified @ 20140527 for custom msgbox btn css
			var tb = $("<div class=\"messager-button\"></div>").appendTo(win);
			var tb1 = $("<div style='width:204px;margin:auto auto;'></div>")
					.appendTo(tb);
			for ( var _268 in _267) {
				$("<div class='btn-operation'></div>").text(_268).bind("click",
						eval(_267[_268])).appendTo(tb1).linkbutton();
			}
		}
		win.window({
			title : _265,
...
(function($) {
	...
	function _4bb(_4bc, _4bd) {
		...
		// wangyan modified @ 20140514 for auto size of datagrid
		if (opts["autoSize"]) {
			// for table size
			var width = getWidth();
			...
			// for table size
			var width = getWidth_1();
			var height = getHeight_1();
	...
	;
	function getWidth() {
		// var inFrame = parent.document != undefined;
		// var parentFrame = $(parent.document);
		var width = 0;// inFrame ? parentFrame.css("width") : 0;
		$(parent.document).find("iframe").each(function() {
			if (this.contentWindow.document == document) {
	...
	function getHeight() {
		// var inFrame = parent.document != undefined;
		// var parentFrame = $(parent.document);
		var height = 0;// inFrame ? parentFrame.css("height") : 0;
		$(parent.document).find("iframe").each(function() {
			if (this.contentWindow.document == document) {
				height = $(this).css("height").replace(/px/g, "");
			}
		});
	...
	$.fn.datagrid = function(_5e6, _5e7) {
		...
		_5e6 = _5e6 || {};
		// wangyan modified @ 20140515 for percent column width
		// var id = $(this).attr("id");
		// var datagridId = id;
		// var datagridOpt = {};
		// $(window).resize(function() {
		// datagridOpt["width"] = getWidth();
		// datagridOpt["height"] = getHeight();
		// $("#" + datagridId).datagrid(datagridOpt);
		// });
		if (_5e6["columns"] && _5e6["columns"][0]) {
			var cols = _5e6["columns"][0];
			var allWidth = getWidth() - 56;
			for ( var i in cols) {
				var json = cols[i];
				if (json["width"]) {
					var width = json["width"];
					if (width.indexOf && width.indexOf("%") != -1) {
						var percent = parseFloat(width.replace(/%/g, ""));
						var offset = allWidth - 1024;
						if (offset < 0) {
							offset = 0;
						} else {
							offset = parseInt((allWidth - 1024) * 0.015);
						}
						json["width"] = parseInt(0.0093 * percent * allWidth
								+ offset);
					}
				}
			}
	...
	function showYScrollBar() {
		$("div.datagrid-body").each(function() {
			var o = $(this);
			var divHeight = parseInt(o.css("height").replace(/px/g, ""));
			var table = o.find("table.datagrid-btable");
			var tableHeight = parseInt(table.css("height").replace(/px/g, ""));
			if (tableHeight > divHeight) {
				var width = parseInt(table.css("width").replace(/px/g, ""));
				o.css("width", (width + 20) + "px");
			}
		});
	}
