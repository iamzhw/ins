...
$(function() {
	img = {
		...
		imagebg_li : document.getElementById("imagebg").getElementsByTagName(
				"li"),
		...
		init : function() {
			...
			for ( var i = 0; i < img.imagebg_li.length; i++) {
				...
				if (i != 0) {
					img.small_pic_html.push("<li onclick='img.play(" + i
							+ ")'><img src='"
							+ img.imagebg_li[i].getAttribute("data-sPic")
							+ "' /></li>");
					img.imagebg.getElementsByTagName("li")[i].style.display = "none";
				} else {
					img.small_pic_html.push("<li onclick='img.play(" + i
							+ ")' class='currently'><img src='"
							+ img.imagebg_li[i].getAttribute("data-sPic")
							+ "' /></li>");
				}
		...
		play : function(i) {
			...
			if (i == img.num || img.animate != null || i > img.count * 3
					|| i < -1) {
				return;
			}
			...
			if (i > (img.count * 3 - 1)) {
				...
				img.small_pic.style.left = small_pic_left + img.imageNumWidth
						* (i - img.count) + "px";
			...
			if (i > img.num) {
				...
				if (parseInt(img.current.style.left) > 500) {
					// 如果活动框到了最右边--完成
					// 小图片向左
					img.animate = setInterval(
							function() {
								if (parseInt(img.small_pic.style.left) > (small_pic_left
										- img.imageNumWidth * (i - img.num) + img.imageNumWidth
										* (i - img.num) / 10)) {
									img.small_pic.style.left = parseInt(img.small_pic.style.left)
											- img.imageNumWidth
											* (i - img.num)
											/ 10 + "px";
									img.imagebg_li[i].style.filter = "alpha(opacity="
											+ (++op) * 11 + ")";
									img.imagebg_li[i].style.opacity = op / 9;
								} else {
									img.small_pic.style.left = small_pic_left
											- img.imageNumWidth * (i - img.num)
											+ "px";
									img.small_pic.getElementsByTagName("li")[i].className = "currently";
					...
					img.animate = setInterval(
							function() {
								if (parseInt(img.current.style.left) < (current_left
										+ img.imageNumWidth * (i - img.num) - img.imageNumWidth
										* (i - img.num) / 10)) {
									img.current.style.left = parseInt(img.current.style.left)
											+ img.imageNumWidth
											* (i - img.num)
											/ 10 + "px";
									// 为了IE联盟
									...
									img.imagebg_li[i].style.filter = "alpha(opacity="
											+ (++op) * 11 + ")"; // "alpha(opacity=100)";

									// 为了火狐部落
									img.imagebg_li[i].style.opacity = op / 9;
								} else {
									img.current.style.left = current_left
											+ img.imageNumWidth * (i - img.num)
											+ "px";
									img.small_pic.getElementsByTagName("li")[i].className = "currently";
									clearInterval(img.animate);
				...
				if (parseInt(img.current.style.left) < 100) {
					...
					img.animate = setInterval(
							function() {
								// console.log(small_pic_left+"+"+
								// img.imageNumWidth*(img.num-i))
								if (parseInt(img.small_pic.style.left) < (small_pic_left
										- img.imageNumWidth * (i - img.num) + img.imageNumWidth
										* (i - img.num) / 10)) {
									img.small_pic.style.left = parseInt(img.small_pic.style.left)
					...
					img.animate = setInterval(
							function() {
								if (parseInt(img.current.style.left) > (current_left
										- img.imageNumWidth * (img.num - i) + img.imageNumWidth
										* (img.num - i) / 10)) {
									img.current.style.left = parseInt(img.current.style.left)
											- img.imageNumWidth
											* (img.num - i)
