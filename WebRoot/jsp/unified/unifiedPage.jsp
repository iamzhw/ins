...
<body style="padding: 3px; border: 0px">
	...
	<div id="tb" style="padding: 5px; height: auto">
		...
		<div class="btn-operation-container">
			...
			<div class="btn-operation" onClick="assignRolePermissions()">角色赋权</div>
			<div class="btn-operation" onClick="assignSoftPermissions()">应用赋权</div>
			<div class="btn-operation" onClick="assignOutSitePermissions()">外力点赋权</div>
			<div style="float: right;" class="btn-operation" onClick="girdManage()">网格管理</div>
	...
	<script type="text/javascript">
		...
		function searchData() {
			...
			$('#dg').datagrid({
				...
				autoSize : true,
				toolbar : '#tb',
				url : webPath + "unifiedPage/query.do",
				queryParams : obj,
				fit : true,
				method : 'post',
				pagination : true,
		...
		function getSonAreaList(operator) {
			...
			$.ajax({
				type : 'POST',
				url : webPath + "unifiedPage/getSonAreaList.do",
				data : {
					areaId : area_id
				},
				dataType : 'json',
			...
			$.ajax({          
				  async:false,
				  type:"post",
				  url :webPath + "unifiedPage/getLocalCompanys.do",
				  data:{areaId:area_id},
				  dataType:"json",
		...
		function addStaff() {
			$('#win_staff').window({
				title : "【新增员工】",
				href : webPath + "unifiedPage/add.do",
				width : 400,
				height : 500,
				zIndex : 2,
				region : "center",
		...
		function editStaff() {
			...
			if (count == 0) {
				...
				$('#win_staff').window({
					title : "【编辑员工】",
					href : webPath + "unifiedPage/edit.do?staff_id=" + staff_id,
					width : 400,
					height : 500,
					zIndex : 2,
					region : "center",
			...
			if (count == 0) {
				...
				$.messager.confirm('系统提示', '您确定要删除员工吗?', function(r) {
					if (r) {
						...
						$('#form').form('submit', {
							url : webPath + "unifiedPage/delete.do",
							onSubmit : function() {
								$.messager.progress();
							},
							success : function() {
			...
			if (pass) {
				$.messager.confirm('系统提示', '您确定要新增员工吗?', function(r) {
					if (r) {
						...
						$.ajax({
							type : 'POST',
							url : webPath + "unifiedPage/save.do",
							data : {
								staff_name : staff_name,
								staff_no : staff_no,
								password : password,
			...
			if (pass) {
				$.messager.confirm('系统提示', '您确定要更新员工吗?', function(r) {
					if (r) {
						...
						$.ajax({
							type : 'POST',
							url : webPath + "unifiedPage/update.do",
							data : {
								staff_id : staff_id,
								staff_name : staff_name,
								staff_no : staff_no,
...
function assignSoftPermissions() {
    ...
    if (count == 0) {
        ...
        $("#win_staff").window({
            title: "【应用赋权】",
            href : webPath + "unifiedPage/assignSoftPermissions.do?staff_id="+staff_id,
			width : 500,
			height : 500,
            zIndex: 2,
            region: "center",
...
function assignRolePermissions() {
    ...
    if (count == 0) {
        ...
        $("#win_staff").window({
            title: "【角色赋权】",
            href : webPath + "unifiedPage/assignRolePermissions.do?staff_id="+staff_id,
			width : 500,
			height : 500,
            zIndex: 2,
            region: "center",
...
var selected = $('#dg').datagrid('getChecked');
    ...
    if (count == 0) {
        ...
        $("#win_staff").window({
            title: "【外力点赋权】",
            href : webPath + "unifiedPage/assignOutSitePermissions.do?staff_id="+staff_id,
			width : 500,
			height : 500,
            zIndex: 2,
            region: "center",
				...
				$.messager.confirm(
								...
								function(r) {
									if (r) {
										...
										$.ajax({
													type : 'POST',
													url : webPath + "unifiedPage/saveAssignOutSitePermissions.do",
													data : obj,
													dataType : 'json',
													success : function(json) {
														if (json.status) {
...
function girdManage(){
    ...
    if (count == 0) {
        ...
        $("#win_staff").window({
            title: "【网格管理】",
            href : webPath + "unifiedPage/gridManage.do?staff_id="+staff_id,
			width : 500,
			height : 500,
            zIndex: 2,
            region: "center",
