...
<body style="padding: 3px; border: 0px">
	...
	<div id="r-result">
		<div style="border-bottom: 1px solid #d2d2d2; padding:10px;">
			<form id="ff">
			    <input type="hidden" id="line_id" value="${lineInfo.LINE_ID}"/>
				<table id="tbl1">

					<tr>
						<td>光缆：</td>
						<td><select class="condition-select" name=""
							id="">
								<option value=''>${gldAndRelay.CABLE_NAME}</option>
						</select></td>
					</tr>
					<tr>
						<td>中继段：</td>
						<td><select class="condition-select" name="relay_id"
							id="relay_id" onchange="getSiteByRelayId(this.value)">
								<option value='${gldAndRelay.RELAY_ID}'>${gldAndRelay.RELAY_NAME}</option>
						</select></td>
					</tr>
					<tr>
						<td width='30%'>长度：</td>
						<td width='60%'><div class='condition-text-container'>
								<input class='condition-text' name='distance' id='distance'
									type='text' value='${lineInfo.DISTANCE}公里' readonly="readonly"/>
							</div></td>
					</tr>
					...
					<tr>
						...
						<td><div class="condition-text-container">
								<input
									...
									type="text" name="line_color" id="line_color"
									onchange="setStrokeColor(this.value)" value="${lineInfo.LINE_COLOR}"/>
							</div>
						</td>
					</tr>
					<tr>
						<td width='30%'>名称：</td>
						<td width='60%'><div class='condition-text-container'>
								<input class='condition-text' name='line_name' id='line_name' value="${lineInfo.LINE_NAME}"
									type='text'  />
							</div>
						</td>
					</tr>
						...
						<td width='60%'><select class='condition-select'
							name='inspect_id' id='inspect_id'>
								<c:forEach items="${localInspactPerson}" var="res">                    
									<option value='${res.STAFF_ID}' <c:if test="${res.STAFF_ID==lineInfo.INSPECT_ID}">selected</c:if>>${res.STAFF_NAME}</option>
								</c:forEach>
		...
		<div style="height: 35px; border-bottom: 1px solid #d2d2d2;">
			<div style="margin-left: 0px;" class="btn-operation"
				onClick="saveCable();">保存</div>
			<div style="margin-left: 0px;" class="btn-operation"
				onClick="history.back();">返回</div>
			<div style="margin-left: 0px;float:right;" class="btn-operation"
				onClick="clearTable();">清除</div>
		</div>
		<div
	...
	//////////////////////////////////////////////
	var sitebak=JSON.parse('${siteInfo}');
...
<%--		clearAll();--%>
	...
	function clearTable(){
<%--		--%>
<%--	strokeColor='#'+'${lineInfo.LINE_COLOR}';--%>
<%--	--%>
<%--	var siteInfo=JSON.parse('${siteInfo}');--%>
<%--	$.each(siteInfo.allSiteList,function(i,item){--%>
<%--		  showPoint(item);--%>
<%--		  --%>
<%--	});--%>
<%--	map.removeOverlay(polylinesss);--%>
...
<%--		map.removeOverlay(polylinesss);--%>
		...
		markersId=[];
		polylinesss.setPath(points);
		$.each(sitebak.allSiteList,function(i,item){
			  showPoint(item);
		});
	}
...
function reactOnMap(thiss) {
	...
	//var distance = BMapLib.GeoUtils.getPolylineDistance(polylinesss).toFixed(2)/1000+"公里";
	var distance = BMapLib.GeoUtils.getPolylineDistance(polylinesss)/1000;
	distance =distance.toFixed(3)+"公里";
    $("#distance").val(distance);
}
	...
	function showTheCurrentSiteOnMap(thiss){
		...
		var marker=new BMap.Marker(point,{icon:checkedIcon});
		map.addOverlay(marker);
	}
	...
	$(function(){
		//展示中继段下的点
				...
				//连接选中的点
				$.each(siteInfo.selectedSiteList,function(i,item){
					...
					overMarker.addEventListener("rightclick", function(e){
			    		...
			    		delMarker(e,item.LONGITUDE,item.LATITUDE,item.SITE_TYPE);
			    	});
					map.addOverlay(overMarker);
					//============
				});
				...
				polylinesss = new BMap.Polyline(points, {
				    strokeColor : strokeColor,//"#9D3071"
				    strokeWeight : 6,
				    strokeOpacity : 0.6
			        });
				...
				//设置地图中心
					 //var point=new BMap.Point(siteInfo.selectedSiteList[0].LONGITUDE,siteInfo.selectedSiteList[0].LATITUDE);
					  //map.centerAndZoom(point, 17);
				map.setViewport(points);
				//原来的点保存在markersId中
					  $.each(siteInfo.selectedSiteList,function(i,item){
							var siteid=item.SITE_ID;
							markersId.push(siteid);
